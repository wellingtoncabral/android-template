...
The app configuration are centralizes inside the [AppConfig.kt](buildSrc/src/main/java/AppConfig.kt) file in the `buildSrc` folder.

## Static Analysis

This template is using [**ktlint**](https://github.com/pinterest/ktlint) with the [ktlint-gradle](https://github.com/jlleitschuh/ktlint-gradle) plugin to format your code.  
To execute the ktlint in the project, please run `./gradlew ktlintCheck` and to reformat all the source code as well as the buildscript you can run the `./gradlew ktlintFormat` gradle task.
...
This template is using [**LeakCanary**](https://square.github.io/leakcanary/) to dramatically reduce `OutOfMemoryError` crashes.
